...
if git grep -I -n -E "$PATTERN" -- . ':(exclude).github/scripts/secret_scan.sh'; then
...
while IFS= read -r commit; do
  if git grep -I -n -E "$PATTERN" "$commit" -- . ':(exclude).github/scripts/secret_scan.sh' >"$tmp_output" 2>/dev/null; then
